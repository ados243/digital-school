...
from .models import Tuteur, Eleve, Inscription, Classe, Annee_Scolaire
...
class EleveForm(FormControlMixin, forms.ModelForm):
    ...
    def __init__(self, *args, ecole=None, **kwargs):
        ...
        self.ecole = ecole
        if ecole:
            self.fields['titeur'].queryset = tuteurs_for_ecole(ecole).order_by('nom', 'prenom')
    ...
    def clean_titeur(self):
        ...
        if self.ecole and tuteur and tuteur.ecole_id != self.ecole.id:
            raise forms.ValidationError("Ce tuteur n'appartient pas à votre établissement.")
        return tuteur
