...
class ConfigWhatsApp(models.Model):
    """Configuration WhatsApp par établissement pour les reçus de paiement."""
    ...
    ecole = models.OneToOneField(
        Ecole, on_delete=models.CASCADE, related_name="config_whatsapp"
    )
    ...
    api_token = models.CharField(
        max_length=255,
        blank=True,
        help_text="Token Ultramsg / Access token Meta",
    )
...
class NotificationWhatsApp(models.Model):
    ...
    ecole = models.ForeignKey(Ecole, on_delete=models.CASCADE, related_name="notif_whatsapp")
    paiement = models.ForeignKey(
        Paiement,
        on_delete=models.SET_NULL,
        null=True,
